...
### Stage 2: `runtime` (ASP.NET chiseled-extra image)
- Base: `mcr.microsoft.com/dotnet/aspnet:9.0-noble-chiseled-extra`
- Ubuntu chiseled image — no shell, no package manager, non-root by default
- **`chiseled-extra` variant** includes ICU and **`tzdata`** (IANA timezone database). The standard `chiseled` variant lacks `tzdata`, which causes `TimeZoneInfo.TryFindSystemTimeZoneById()` to fail and all user-facing dates/times to display in UTC instead of the user's local timezone.
